...
public class World implements Constant {
	...
	public String addPlayer(String name, double screenWidth, double screenHeight) {
		synchronized (lock) {
			...
			String playerId = gameObjectCollection.addPlayer(name, x, y, size, PLAYER_COLOR, screenWidth, screenHeight);
			return playerId;
		}
	}
	...
	public void addBullet(Player player, double destinationX, double destinationY) {
		...
		synchronized (lock) {
			...
			gameObjectCollection.addBulletToQueue(playerX, playerY, destinationX, destinationY, BULLET_SIZE,
					BULLET_COLOR, player);
		}
